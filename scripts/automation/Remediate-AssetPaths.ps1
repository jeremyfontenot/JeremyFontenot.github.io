...
$assetFiles = Get-ChildItem -LiteralPath $RootPath -Recurse -File | Where-Object {
  @('.html', '.htm', '.md', '.markdown', '.css', '.js', '.xml', '.json') -contains $_.Extension.ToLowerInvariant() -and $_.FullName -notmatch '\\internal\\reports\\phase2\\'
}
...
$reportPath = Join-Path $RootPath 'internal\reports\phase2\asset-path-remediation-plan.csv'
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $reportPath) | Out-Null
$plan | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $reportPath
Write-Host ('Asset path remediation plan written to ' + $reportPath)
