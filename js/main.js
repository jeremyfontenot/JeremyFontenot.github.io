(function () {
  ...
  // Navigation active-link handling: update `aria-current="page"` on click and on load
  (function manageNavCurrent() {
    const navLinks = Array.from(document.querySelectorAll('.nav-link'));
    if (!navLinks.length) return;

    const clearCurrent = () => navLinks.forEach(l => l.removeAttribute('aria-current'));
    const setCurrent = (el) => { clearCurrent(); if (el) el.setAttribute('aria-current', 'page'); };

    // Click handler: mark clicked link as current (works for same-page anchors and navigation)
    ...
    // On load: prefer exact href (including hash), else match pathname
    try {
      const hrefNow = location.href;
      let match = navLinks.find(l => {
        try { return new URL(l.href, location.href).href === hrefNow; } catch { return false; }
      });
      if (!match) {
        match = navLinks.find(l => {
          try { return new URL(l.href, location.href).pathname.replace(/\/$/, '') === location.pathname.replace(/\/$/, ''); } catch { return false; }
        });
      }
  ...
  // Background imagery toggle: remembers preference in localStorage
  ...
  try {
    ...
    // Default to disabled (remove background images) unless user opted in.
